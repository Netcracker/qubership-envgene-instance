...
def get_content_form_file(file: str) -> Any:
    try:
        ...
        raise ValidationError(
            ErrorMessages.INVALID_YAML_FILE.format(file=file, e=str(e)),
...
def get_nested_target_key(yaml_data: dict, path: str) -> str:
    ...
    for part in path.split("."):
        ...
        if not isinstance(current, dict) or key not in current:
            raise ValueError(
                ErrorMessages.MISSING_KEY.format(key=key, current=current),
            ...
            if not isinstance(current, list):
                raise ValueError(
                    ErrorMessages.INVALID_DATA_TYPE.format(
                        expected="list", value=key[index], type=type(current)
                    ),
                    ErrorCodes.INVALID_DATA_TYPE_CODE,
                )
            if index >= len(current):
                raise ValueError(
                    ErrorMessages.OUT_OF_RANGE.format(index=index, key=key),
                    ErrorCodes.OUT_OF_RANGE_CODE,
                )
