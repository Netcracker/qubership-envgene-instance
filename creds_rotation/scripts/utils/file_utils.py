...
async def read_yaml_file(path: str) -> tuple[str, dict]:
    try:
        ...
        raise ValidationError(
            ErrorMessages.INVALID_YAML_FILE.format(file=path, e=str(e)),
    ...
    try:
        with os.scandir(path) as entries:
            ...
            for dir_path in dirs_to_process:
                scandir_recursive_optimized(
                    dir_path, ns_files, definition_files, env_creds_files
                )
...
def scandir_recursive(
    path: str, ns_files: Set[str], definition_files: Set[str], env_creds_files: Set[str]
) -> None:
...
def scan_and_get_yaml_files(env_dir: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
