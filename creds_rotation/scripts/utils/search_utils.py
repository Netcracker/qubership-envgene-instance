...
def find_in_yaml(
    ...
    for context in CONTEXT_MAP.values():
        ...
        if params and search_pattern in str(params):
            affected_params = find_matching_keys(
                params, search_pattern, is_target, context, target_key, target_context
            )
            if affected_params:
...
def get_ns_content(
    yaml_content_map: Dict[str, Dict[str, Any]], namespace: str, env_name: str
...
def get_app_and_ns(
    filename: str, content: dict, entity_files_map: Dict[str, Dict[str, Any]]
        ...
        for key in param_keys:
            affected = AffectedParameter(
                ...
                shared_cred_filepath=[
                    trim_path_from_environments(p) for p in shared_cred_files
                ],
                cred_id=cred_id,
            )
