...
from envgenehelper import crypt, dump_as_yaml_format, openYaml, writeYamlToFile
...
pattern = re.compile(
    r"\$\{creds\.get\([\"']([^\"']+)[\"']\)\.(username|password|secret)\}"
)
...
def collect_shared_credentials(env_files_map: Dict[str, Any]) -> Set[str]:
    ...
    for file_content in env_files_map.values():
        creds = file_content.get("envTemplate", {}).get(
    ...
    if shared_cred_names:
        logger.info(
            f"✅ Inventory shared master creds list collected from all envs:\n{dump_as_yaml_format(sorted(shared_cred_names))}"
        )
...
def extract_credential(
    target_key: str, yaml_content: Dict[str, Any]
    ...
    if target_key in yaml_content:
        ...
        raise ValidationError(
            ErrorMessages.PARAM_NOT_FOUND.format(target_key=target_key),
            ErrorCodes.INVALID_INPUT_CODE,
        )

    if not isinstance(value, str):
        raise ValueError(
            ErrorMessages.INVALID_DATA_TYPE.format(
                expected="string", value=target_key, type=type(value)
            ),
...
def read_shared_cred_files(
    shared_creds: Set[str],
    cluster_dir: str,
    work_dir: str,
    is_encrypted: str,
    public_key: str,
    ...
    dirs_to_scan = [
        f"{work_dir}/environments",
        f"{work_dir}/environments/credentials",
        f"{work_dir}/environments/Credentials",
    ]
    allowed_exts = (".yml", ".yaml", ".json")
    source_path = Path(cluster_dir).resolve()
    scan_dir_for_creds(
        source_path, shared_creds, shared_creds_files_set, is_encrypted, public_key
    )
    files = [
        entry.path
        ...
        if entry.is_file()
        and entry.name.endswith(allowed_exts)
        and os.path.splitext(entry.name)[0] in shared_creds
    ]
    shared_creds_files_set.update(files)
    shared_content_map = read_env_cred_files(
        shared_creds_files_set, is_encrypted, public_key
    )
    return shared_content_map


def scan_dir_for_creds(
    path: Path,
    shared_creds: set,
    shared_creds_files_set: List[str],
    is_encrypted: str,
    public_key: str,
):
...
def decrypt_and_get_content(is_encrypted, public_key, filepath):
    if is_encrypted:
        return decrypt_file(
            ...
            True,
            "SOPS",
            ErrorMessages.FILE_DECRYPT_ERROR,
    ...
    try:
        ...
        raise ValidationError(
            ErrorMessages.FILE_READ_ERROR.format(file=filepath, e=str(e)),
            error_code=ErrorCodes.INVALID_CONFIG_CODE,
        )


def decrypt_file(
    envgene_age_public_key, file_path, in_place, crypt_backend, error_msg, error_code
):
    try:
        return crypt.decrypt_file(
            ...
            public_key=envgene_age_public_key,
            crypt_backend=crypt_backend,
        )
    except Exception as e:
        raise ValidationError(
            error_msg.format(file=file_path, e=str(e)), error_code=error_code
        )


def update_cred_content(processed_cred_and_files: Dict[str, List[CredMap]]):
    updated_files: Dict[str, List[Dict[str, Any]]] = {}
    original_files: Dict[str, List[Dict[str, Any]]] = {}

    logger.info(
        f"Preparing updates for {len(processed_cred_and_files)} credential files"
    )
    ...
    for cred_file, updates in processed_cred_and_files.items():
        ...
        for update in updates:
            ...
            logger.debug(
                f"Updating credential '{cred_id}' field '{cred_field}' in file '{cred_file}'"
            )

            data_block = content[cred_id].get("data")
            if not data_block:
                raise ValidationError(
                    ErrorMessages.MISSING_BLOCK.format(
                        block="data", cred_id=cred_id, cred_file=cred_file
                    ),
                    ErrorCodes.INVALID_INPUT_CODE,
                )
...
def write_updated_cred_into_file(
    updated_files, original_files, is_encrypted, envgene_age_public_key
):
    try:
        ...
        logger.error(
            f"Updation of credential file failed {e} \n Hence reverting the change"
        )
        update_file(original_files, is_encrypted, envgene_age_public_key)
        logger.info(
            "No creds changes are updated. Files are restored to original state"
        )
...
def update_file(files_to_update, is_encrypted, envgene_age_public_key):
    ...
    with ProcessPoolExecutor(
        max_workers=min(os.cpu_count(), len(files_to_update))
    ) as executor:
        for cred_file, creds in files_to_update.items():
            futures.append(
                executor.submit(
                    write_and_encrypt_task,
                    cred_file,
                    creds,
                    is_encrypted,
...
def write_and_encrypt_task(cred_file, creds, is_encrypted, public_key):
    ...
    if is_encrypted:
        try:
            ...
            raise ValidationError(
                ErrorMessages.FILE_ENCRYPT_ERROR.format(file=cred_file, e=str(e)),
                error_code=ErrorCodes.INVALID_CONFIG_CODE,
            )
