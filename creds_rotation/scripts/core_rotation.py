...
from envgenehelper.errors import ReferenceError, ValidationError
from models import CredMap, ParameterReference, PayloadEntry, RotationResult
...
def process_entry_in_payload(
    ...
    if entry.application and entry.context == "pipeline":
        raise ValidationError(
            ErrorMessages.INVALID_CONTEXT.format(param_key=entry.parameter_key),
            error_code=ErrorCodes.INVALID_INPUT_CODE,
        )

    logger.info(
        f"Processing namespace={entry.namespace}, application={entry.application}, param_key={entry.parameter_key}, context={entry.context}"
    )
    # Get target application or namespace file
    if entry.application:
        target = get_app_content(
            entity_files_map, entry.namespace, entry.application, env
        )
    else:
    ...
    if not target:
        raise ReferenceError(
            ErrorMessages.ENTITY_FILE_NOT_FOUND.format(param_key=entry.parameter_key),
    ...
    if param_type not in target_yaml:
        raise ValidationError(
            ErrorMessages.MISSING_CONTEXT.format(
                context=entry.context,
                target_file=target_file,
                param_key=entry.parameter_key,
            ),
            error_code=ErrorCodes.INVALID_INPUT_CODE,
        )

    sub_content = target_yaml[param_type]
    value_to_search, cred_id, cred_field = extract_credential(
        entry.parameter_key, sub_content
    )

    if cred_field not in {"username", "password", "secret"}:
        raise ValidationError(
            ErrorMessages.INVALID_CRED_FIELD.format(cred_field=entry.credential_field),
            ErrorCodes.INVALID_CONFIG_CODE,
        )

    if not value_to_search or not cred_id:
        raise ReferenceError(
            ErrorMessages.MISSING_CRED.format(
                target_file=target_file, context=entry.context
            ),
            error_code=ErrorCodes.INVALID_INPUT_CODE,
        )
    ...
    for cred_file in all_cred_files:
        shared_content = shared_cred_content.get(cred_file) or env_cred_content.get(
            cred_file
        )
        cred_map = CredMap(
            cred_id=cred_id,
    ...
    logger.info(
        f"✅ Processed param {entry.parameter_key}, affected count = {len(affected)}"
    )
    if affected:
    ...
    logger.info(
        f"No affected parameters found for the given target key {entry.parameter_key}"
    )
    return None
